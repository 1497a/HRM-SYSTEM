 ...
 * Singleton pattern. Sử dụng AttendanceRepository (JDBC).
 */
public class AttendanceService {
    ...
    private AttendanceService() {
        this.repository = AttendanceRepository.getInstance();
    }
     ...
     */
    public ServiceResult<CaLam> saveCaLam(CaLam caLam) {
        if (caLam.getMaCaLam() == null || caLam.getMaCaLam().trim().isEmpty()) {
            return ServiceResult.error("Mã ca làm không được để trống.");
        }
        if (caLam.getTenCaLam() == null || caLam.getTenCaLam().trim().isEmpty()) {
            return ServiceResult.error("Tên ca làm không được để trống.");
        }
        if (caLam.getGioBatDau() == null || caLam.getGioKetThuc() == null) {
            return ServiceResult.error("Giờ bắt đầu và kết thúc không được để trống.");
        }
        try {
            repository.saveCaLam(caLam);
            return ServiceResult.success(caLam, "Đã lưu ca làm '" + caLam.getTenCaLam() + "'.");
        } catch (Exception e) {
            return ServiceResult.error("Lỗi lưu ca làm: " + e.getMessage());
        }
    }
    ...
    public ServiceResult<CaLam> themCaLam(String ma, String ten, String gioBD, String gioKT, double sg, boolean ot) {
        ...
        if (existing != null) return ServiceResult.error("Mã ca đã tồn tại.");
        try {
            ...
            return ServiceResult.success(ca, "Đã thêm ca làm '" + ten + "'.");
        } catch (Exception e) {
            return ServiceResult.error("Định dạng giờ không hợp lệ (HH:mm).");
        }
    }
    ...
    public ServiceResult<CaLam> suaCaLam(String ma, String ten, String gioBD, String gioKT, double sg, boolean ot) {
        ...
        if (ca == null) return ServiceResult.error("Không tìm thấy ca làm.");
        try {
            ...
            ca.setChoPhepLamThem(ot);
            repository.saveCaLam(ca);
            return ServiceResult.success(ca, "Đã cập nhật ca làm.");
        } catch (Exception e) {
            return ServiceResult.error("Định dạng giờ không hợp lệ.");
        }
    }
    ...
    public ServiceResult<Void> xoaCaLam(String ma) {
        ...
        return ServiceResult.success(null, "Đã ngừng ca làm '" + ca.getTenCaLam() + "'.");
    }
     ...
     */
    public ServiceResult<ChamCong> checkIn(int maNV, String maCaLam, String phuongThuc) {
        LocalDate today = LocalDate.now();
        if (repository.findChamCongByNVAndNgay(maNV, today) != null) {
            return ServiceResult.error("Bạn đã check-in hôm nay rồi.");
        }
        CaLam caLam = repository.findCaLamById(maCaLam);
        if (caLam == null) {
            return ServiceResult.error("Ca làm không hợp lệ.");
        }
        ...
        return ServiceResult.success(cc, "Check-in thành công lúc " + now.toLocalTime().toString().substring(0, 5) + ".");
    }
    ...
    public ServiceResult<ChamCong> checkOut(int maNV) {
        ...
        if (cc == null) {
            return ServiceResult.error("Bạn chưa check-in hôm nay.");
        }
        if (cc.daCheckOut()) {
            return ServiceResult.error("Bạn đã check-out rồi.");
        }
        ...
        return ServiceResult.success(cc, "Check-out thành công. Tổng giờ làm: "
     ...
     */
    public ServiceResult<ChamCong> themChamCongThuCong(int maNV, LocalDate ngay, String maCaLam,
                                                        LocalDateTime gioVao, LocalDateTime gioRa,
                                                        ChamCong.TrangThai trangThai, String ghiChu) {
        CaLam caLam = repository.findCaLamById(maCaLam);
        if (caLam == null) {
            return ServiceResult.error("Mã ca làm không hợp lệ.");
        }
        if (repository.findChamCongByNVAndNgay(maNV, ngay) != null) {
            return ServiceResult.error("Nhân viên đã có bản ghi chấm công ngày " + ngay + ".");
        }
        ...
        repository.saveChamCong(cc);
        return ServiceResult.success(cc, "Đã thêm chấm công thủ công cho ngày " + ngay + ".");
    }
    ...
    public ServiceResult<DangKyLamThem> taoDonLamThem(int maNV, LocalDate ngay, double soGio, String lyDo) {
        if (soGio <= 0 || soGio > 8) {
            return ServiceResult.error("Số giờ OT phải từ 0.5 đến 8 giờ.");
        }
        if (lyDo == null || lyDo.trim().isEmpty()) {
            return ServiceResult.error("Vui lòng nhập lý do làm thêm.");
        }
        ...
        if (tongOTThang + soGio > OT_TONG_TOI_DA_THANG) {
            ...
            return ServiceResult.error(String.format(
        ...
        return ServiceResult.success(dk, "Đã tạo đơn đăng ký làm thêm " + soGio + " giờ.");
    }

    /**
     * Duyệt đơn OT.
     */
    public ServiceResult<DangKyLamThem> duyetDonLamThem(int maDK, int nguoiDuyetId) {
        DangKyLamThem dk = repository.findById(maDK);
        if (dk == null) {
            return ServiceResult.error("Không tìm thấy đơn OT.");
        }
        if (!dk.dangChoDuyet()) {
            return ServiceResult.error("Đơn đã được xử lý rồi.");
        }
        LocalDateTime now = LocalDateTime.now();
        ...
        dk.setNgayDuyet(now);
        return ServiceResult.success(dk, "Đã duyệt đơn làm thêm.");
    }
    ...
    public ServiceResult<DangKyLamThem> tuChoiDonLamThem(int maDK, int nguoiDuyetId) {
        ...
        dk.setNguoiDuyet(nguoiDuyetId);
        dk.setNgayDuyet(now);
        return ServiceResult.success(dk, "Đã từ chối đơn làm thêm.");
    }
    ...
    public ServiceResult<Void> capNhatHeSoOT(int maDK, double heSo) {
        if (heSo <= 0) return ServiceResult.error("Hệ số OT phải lớn hơn 0.");
        DangKyLamThem dk = repository.findById(maDK);
        if (dk == null) return ServiceResult.error("Không tìm thấy đơn OT.");
        repository.updateHeSoOT(maDK, heSo);
        return ServiceResult.success(null, "Đã cập nhật hệ số OT thành x" + heSo + ".");
    }
    ...
    public ServiceResult<CauHinhPhuCap> themCauHinhPC(ThanhPhanLuong.Loai loai, String tenKhoan,
        ...
        if (tenKhoan == null || tenKhoan.trim().isEmpty()) {
            return ServiceResult.error("Tên khoản không được để trống.");
        }
        if (giaTri <= 0) return ServiceResult.error("Giá trị phải lớn hơn 0.");
        CauHinhPhuCap pc = new CauHinhPhuCap(loai, tenKhoan.trim(), kieuTinh, giaTri, nguon);
        repository.insertCauHinhPC(pc);
        return ServiceResult.success(pc, "Đã thêm khoản '" + tenKhoan + "'.");
    }
    ...
    public ServiceResult<CauHinhPhuCap> suaCauHinhPC(int maPC, ThanhPhanLuong.Loai loai, String tenKhoan,
        ...
        if (pc == null) return ServiceResult.error("Không tìm thấy khoản phụ cấp.");
        if (tenKhoan == null || tenKhoan.trim().isEmpty()) {
            return ServiceResult.error("Tên khoản không được để trống.");
        }
        if (giaTri <= 0) return ServiceResult.error("Giá trị phải lớn hơn 0.");
        ...
        return ServiceResult.success(pc, "Đã cập nhật khoản '" + tenKhoan + "'.");
    }
    ...
    public ServiceResult<Void> xoaCauHinhPC(int maPC) {
        ...
        return ServiceResult.success(null, "Đã ngừng khoản '" + pc.getTenKhoan() + "'.");
    }
}
