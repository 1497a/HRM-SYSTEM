...
/**
 * Generic wrapper cho kết quả trả về từ BUS layer.
 *
 ...
 *
 * KetQua giải quyết tất cả:
 *   KetQua<ChamCong> result = service.checkIn(nvId);
 *   if (result.isSuccess()) {
 ...
 *   KetQua<ChamCong>       → T = ChamCong
 *   KetQua<DangKyLamThem>  → T = DangKyLamThem
 *   KetQua<Void>           → T = Void (không cần data)
 *
